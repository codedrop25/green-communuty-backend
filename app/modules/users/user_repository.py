...
class UserRepository:
    ...
    def exists_by_email(self, user_email: str) -> bool:
        ...
        statement = select(func.count()).select_from(User).where(User.user_email == user_email)
    ...
    def list_paginated(self, params: PageParams) -> tuple[list[User], int]:
        ...
        statement = (
            select(User).order_by(User.user_id.desc()).offset(params.offset).limit(params.limit)
        )
    ...
    def add(self, user: User) -> User:
        ...
        덕분에 Service 가 이어지는 작업에서 `user.user_id` 를 쓸 수 있으면서도
